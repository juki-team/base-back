...
describe('Side effects', function () {
    it('tsconfig out dir can not be changed', function () {
        const config = tsconfig_1.getTSConfig(true, { compilerOptions: { outDir: 'other' } });
    ...
    it('should find mainDistFile for ts', function () {
      ...
      `
            ...
            const files = yield compile_1.collectDistFiles({ directory, srcTestFiles: [] });
            const testFiles = [];
            const main = compile_1.findMainFile({ directory, main: 'TrapFocus/index.ts' }, files);
    ...
    it('should find mainDistFile for tsx', function () {
      ...
      `
            ...
            const files = yield compile_1.collectDistFiles({ directory, srcTestFiles: [] });
            const testFiles = [];
            const main = compile_1.findMainFile({ directory, main: 'TrapFocus/index.tsx' }, files);
            chai_1.expect(main).to.equal('TrapFocus/index.js');
            yield bit_envs_common_build_component_1.removeWorkspace(directory);
        });
    });
    it('should find mainDistFile for js', function () {
      ...
      `
            ...
            helper.command.runCmd('bit build trap-focus', directory);
            const files = yield compile_1.collectDistFiles({ directory, srcTestFiles: [] });
            const main = compile_1.findMainFile({ directory, main: 'TrapFocus/index.js' }, files);
            chai_1.expect(main).to.equal('TrapFocus/index.js');
            yield bit_envs_common_build_component_1.removeWorkspace(directory);
        });
    });
